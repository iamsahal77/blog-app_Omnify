// Navigation component with responsive design and authentication state
import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';

const Navbar = () => {
    // State for mobile menu toggle and authentication status
                ...
                <div className="flex items-center justify-between h-16">
                    ...
                    <div className="hidden md:block">
                        <div className="ml-10 flex items-baseline space-x-4">
                            ...
                            {isAuthenticated && (
                                <Link to="/create" className="text-indigo-600 hover:bg-indigo-50 px-3 py-2 rounded-md text-sm font-medium">Create Post</Link>
                            )}
                        </div>
                    </div>
            ...
            {isMenuOpen && (
                <div className="md:hidden">
                    <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                        ...
                        {isAuthenticated && (
                            <Link to="/create" className="text-indigo-600 hover:bg-indigo-50 block px-3 py-2 rounded-md text-base font-medium">Create Post</Link>
                        )}
                    </div>
