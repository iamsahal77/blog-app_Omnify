...
-- Run this in your Supabase SQL Editor to fix the 409 conflict
...
DROP POLICY IF EXISTS "Allow public read access to user profiles" ON auth_users;
DROP POLICY IF EXISTS "Allow users to update their own profile" ON auth_users;
DROP POLICY IF EXISTS "Allow users to insert their own profile" ON auth_users;
